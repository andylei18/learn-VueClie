...
<template>
  <div>
    <!-- 初始化炸花 -->
    <co-ver :show="loadding.show"></co-ver>

    <div id="applist">
      <div class="page">
        <!--加载框-->
        <load-ing :show="this.$route.router.app.progressbar"></load-ing>
      ...
      <!--左侧菜单栏-->
      <left-menu :show.sync="menu.show" :list.sync="menu.list"></left-menu>
    </div>
  </div>
</template>
<script>
  ...
  export default {
    ...
    methods: {
      ...
      getAjax () {
        var _self = this;
        var params = $.param(_self.searchKey);
